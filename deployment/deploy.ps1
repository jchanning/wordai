# Canonical WordAI deployment script for Oracle Cloud
...
# Load configuration
$configFile = Join-Path $PSScriptRoot 'deployment-config.ps1'
...
Write-Step "Step 1: Building application for cloud (Java 17)..."
...
Write-Step "Step 2: Uploading JAR to server..."
$jarFile = Get-ChildItem "$PSScriptRoot\..\target\wordai-*.jar" | Sort-Object LastWriteTime -Descending | Select-Object -First 1
if (-not $jarFile) {
...
Write-Step "Step 3: Uploading configuration files..."
scp -i $KEY "$PSScriptRoot\application-prod.properties" opc@${IP}:~/wordai-app/wordai.properties
scp -i $KEY "$PSScriptRoot\setup-dictionaries.sh" opc@${IP}:~/wordai-app/
scp -i $KEY "$PSScriptRoot\wordai.service" opc@${IP}:~/

# Step 4: Extract dictionaries and restart
Write-Step "Step 4: Setting up dictionaries and restarting service..."
ssh -i $KEY opc@$IP "cd ~/wordai-app; chmod +x setup-dictionaries.sh; tr -d '\r' < setup-dictionaries.sh | bash; if [ -f ~/wordai.service ]; then sudo cp ~/wordai.service /etc/systemd/system/wordai.service; sudo systemctl daemon-reload; fi; sudo systemctl restart wordai; sleep 3; sudo systemctl status wordai --no-pager"
